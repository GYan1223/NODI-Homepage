...
访问统计使用页面末尾隐藏加载的 Flag Counter 图片请求。主页不显示计数器组件，统计结果在对应的 Flag Counter 后台查看。浏览器或网络工具拦截 `flagcounter.com` 时，该次访问可能无法计入。
...
新增图片时应使用小写英文文件名和连字符，避免空格、冒号及仅大小写不同的文件名。提交前应清除 EXIF/GPS 元数据，并控制图片尺寸；成员头像最长边建议不超过 384px、单张不超过 128 KiB，活动照片最长边建议不超过 1800px。
